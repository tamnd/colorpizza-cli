...
describes what users get. Pick a format with `-o`, or let color choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
color <command> -o table   # aligned columns for reading
color <command> -o jsonl   # one JSON object per line, for piping
color <command> -o json    # a single JSON array
color <command> -o csv     # spreadsheet friendly
color <command> -o tsv     # tab-separated
color <command> -o url     # just the URL column
color <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
color <command> --fields id,title,url
```
...
```bash
color <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
color <command>            # a table, because this is a terminal
color <command> | wc -l    # JSONL, because this is a pipe
```
