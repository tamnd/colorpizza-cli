// Package cli assembles the color command tree from the color
...
// NewApp assembles the kit application from the color domain. The
...
//
// To add a command, declare it in color/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := color.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(color.Domain{}).Register(app)
